...
float opRep( vec3 p, vec3 c )
{
	...
	c = c+offset;
	vec3 q = mod(p,c)-0.5*abs(c);
	return sdTorus( q , (vec2(0.2, 0.04*(0.0027)+0.05)));
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	// Ray from UV
	vec2 uv = fragCoord.xy * 2.0 / resolution.xy - 1.0;
	...
	for (int r = 0; r < rayCount; ++r)
	{
		...
		if (d < rayEpsilon || t > rayMax)
		{
			...
			c = mix(lightColor, shadowColor, float(r) / float(rayCount));
			
			// Sky color from distance
			c = mix(c, skyColor, smoothstep(rayMin, rayMax, t));
			break;
		}
